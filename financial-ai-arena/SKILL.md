...
```bash
cd skills/financial-ai-arena
python scripts/serve_web.py
```

**一键起后端 + 内嵌配置页（Electron）**：进入 `skills/financial-ai-arena/desktop/`，执行 `npm install` 后 `npm start`，或双击 `desktop/start_desktop.bat`。窗口工具栏可启动/停止由本进程拉起的 `serve_web.py`（详见该目录 `README.txt`）。
...
加载顺序（`arena_run` / `serve_web` / `suggest_personas` 共用 **`scripts/arena_dotenv.py`**）：若存在 **仓库根** `.env`（如 `jingrong_skill/.env`）先读入，再读 **本 skill 根** `.env`（`skills/financial-ai-arena/.env`），后者**覆盖**同名变量；便于把 Key 只写在仓库根一份。
...
将 **`skills/financial-ai-arena/`** 作为 skill 根目录打包（zip / 私仓 / 附件），至少包含：
...
1. 解压到宿主工作区的 `skills/financial-ai-arena/`（与其它 skill 并列时路径与 `SKILL.md` 中「并列技能」一致）。
